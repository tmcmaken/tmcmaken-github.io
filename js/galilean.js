...
(function() {
    ...
    const canvas = document.getElementById('canvasGal');
...
function applyRadialGradientMask(ctx) {
  ...
  ctx.globalCompositeOperation = 'source-over';
    ...
    function drawGrid() {
      ...
      // Translate and skew the entire grid
      ctx.translate(-boostParameter * offsetY + w / 2, offsetY);
      ctx.transform(1, 0, -boostParameter, 1, w / 2 * boostParameter, 0);
...
// Set the stroke color to yellow
  ...
  ctx.fillText('r', 0.9 * w, 0.53 * h);
    ...
    document.getElementById('resetButtonGal').addEventListener('click', handleResetButtonClick);
